...
/*   Updated: 2023/02/05 16:55:08 by aparvin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	stash_arr[fd] = ft_read(fd, stash_arr[fd]);
	if (!stash_arr[fd] || fd < 0)
		return (NULL);
	n_idx = ft_n_idx(stash_arr[fd]);
	if (!n_idx)
	{
		line = ft_substr(stash_arr[fd], 0, ft_strlen(stash_arr[fd]));
		free(stash_arr[fd]);
		stash_arr[fd] = NULL;
		return (line);
	}
	line = ft_substr(stash_arr[fd], 0, n_idx - stash_arr[fd] + 1);
	if (!line)
	{
		free (stash_arr[fd]);
		stash_arr[fd] = NULL;
	...
	stash_arr[fd] = ft_memmove(stash_arr[fd], n_idx + 1, \
			(ft_strlen(n_idx + 1) + 1));
	return (line);
}
